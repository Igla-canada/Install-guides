...
type Item = { id: string; title: string; sub: string; status: string };
...
export default function ExportSelector({ items }: { items: Item[] }) {
  ...
  const all = () => setSel(new Set(items.map((i) => i.id)));
  ...
  return (
    <div>
      ...
      <ul className="mt-4 overflow-hidden rounded-xl border border-zinc-200 bg-white">
        ...
        {items.length === 0 && (
          <li className="px-4 py-6 text-center text-sm text-zinc-400">No guides to export.</li>
        )}
      </ul>
    </div>
  );
